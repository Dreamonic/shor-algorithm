...
from src.topology.parallel_gates.qft import qft, qft_inverse
from src.topology.parallel_gates.rotate import calculate_phase
from src.topology.parallel_gates.simple import CRz, CNOT
...
def Cadd(eng, circuit, c, qubits, value):
    length = len(qubits)
    for n in range(length - 1, -1, -1):
        circuit.apply_ld_routine(c, qubits, Cadd_routine, eng=eng, circuit=circuit, value=value, n=n, qubits=qubits)


def Cadd_routine(eng, circuit, ctrl, tgt, value, n, qubits):
    i = qubits.index(tgt) + 1
    if n - i + 1 >= 0:
        c = value & (1 << (n - i + 1))
        if c > 0:
            CRz(eng, circuit, ctrl, tgt, calculate_phase(i))
